...
Data: CSVS about house sale price, with 79 variables 

Method: regression and machine learning algorithms
...
num_var = train.select_dtypes(np.number)
corr_price = num_var.corr()["SalePrice"].sort_values(ascending=False)
...
sns.heatmap(num_var[corr_price.index[:10]].corr(), annot=True, cmap="GnBu")
"""
# ==========================================================================
...
print("the count missing train is " + str(count_missing_train.count()))
...
train_num.fillna(train_num.mode().iloc[0], inplace=True)


"""
"""
This is not necessary since train_cat doesnt exist
...
percent_missing_train = (train_set.isnull().sum() / train_set.isnull().count()
                         * 100).sort_values(ascending=False)
#
print("still missing is " + str(percent_missing_train.index[percent_missing_train > 0].tolist()))
